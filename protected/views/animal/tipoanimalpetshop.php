<div ng-controller="TipoanimalpetshopController">
	<div class="row">
	    <div class='col-xs-12'>
	...
	<div class="row">
		<div class='col-xs-6'>
	    	<h4>Lista de tipo de animal ({{listatipoanimalpetshop.length}})</h4>
	    </div>
	    <div class='col-xs-6'>
	    	<div class="form-group text-right">
				<button type="submit" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#modalCadastroTipoanimalpetshop" ng-click="nova()">
                ...
                <?php 
					Yii::import('application.views.modal.modalCadastroTipoanimalPetshop',true);
				?>
	    	</div>
	    </div>
	    <div class='col-xs-12'>
			...
			<div class="table-responsive table-config">
				<table class="table table-hover table-striped">
					<tbody>
						<tr class='hand' ng-repeat='tipoanimalpetshop in listatipoanimalpetshop | filter:busca | orderBy:predicate:reverse'>
							<th class='text-center' scope='row' width='10%'>{{tipoanimalpetshop.id}}</th>
							<td class='text-left text-capitalize' >{{tipoanimalpetshop.nome}}</td>
							<td class='text-center text-danger' width='10%'><i class='glyphicon glyphicon-trash' ng-click='deletar(tipoanimalpetshop)'></i></td>
						</tr>
					</tbody>
				</table>
			</div>
